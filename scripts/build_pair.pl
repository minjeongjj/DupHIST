...
sub get_ks {
    ...
    return "NAN" if !defined $g1 || !defined $g2;
    return "NAN" if $g1 eq '' || $g2 eq '';

    return "0" if $g1 eq $g2;  # optional self Ks

    return "NAN" unless exists $idx_of{$g1} && exists $idx_of{$g2};
    ...
    return "NAN" if !defined $matrix[$row] || !defined $matrix[$row][$col];
