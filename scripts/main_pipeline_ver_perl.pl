...
my $cds_file   = $config->{required_option}{cds_fasta};
my $pep_file   = $config->{required_option}{pep_fasta};
my $group_file = $config->{required_option}{group_info};

if (!-e $cds_file || -s $cds_file == 0) {
    die "\n[ERROR] Required file missing or empty: 'cds_fasta' -> $cds_file\nPlease check your config file and try again.\n\n";
}

if (!-e $pep_file || -s $pep_file == 0) {
    die "\n[ERROR] Required file missing or empty: 'pep_fasta' -> $pep_file\nPlease check your config file and try again.\n\n";
}

if (!-e $group_file || -s $group_file == 0) {
    die "\n[ERROR] Required file missing or empty: 'group_info' -> $group_file\nPlease check your config file and try again.\n\n";
}


my $custom_tree_list = $config->{topology_option}{custom_tree_list};

if ($custom_tree_list ne "NA") {
    
    if (!-e $custom_tree_list || -s $custom_tree_list == 0) {
        die "\n[ERROR] Custom tree list file missing or empty: $custom_tree_list\nPlease check 'custom_tree_list' in your config.\n\n";
    }

    open(my $list_fh, "<", $custom_tree_list) or die "[CRITICAL] Cannot open $custom_tree_list: $!\n";
    ...
    while (my $line = <$list_fh>) {
        ...
        if (!$tree_file || !-e $tree_file || -s $tree_file == 0) {
            close($list_fh);
            die "\n[ERROR] Precomputed tree file missing or empty for Group '$group':\n" .
                "  -> Specified Path: '$tree_file'\n" .
                "Please fix the broken path in your custom tree list file.\n\n";
        }
    }
    close($list_fh);
}

my $midpoint_rooting = $config->{topology_option}{midpoint_rooting};
$midpoint_rooting =~ s/\s*#.*// if defined $midpoint_rooting;

## midpoint rooting option validation
if (!defined $midpoint_rooting || ($midpoint_rooting ne "Y" && $midpoint_rooting ne "N")) {
    my $val = defined $midpoint_rooting ? "'$midpoint_rooting'" : "undef";
    die "\n[ERROR] Invalid value for 'midpoint_rooting' -> $val\n" .
        "Please specify either 'Y' or 'N' in your config file.\n\n";
}
...
if ($step == 1)
{
	...
	## custom tree status	
	if ($config->{topology_option}{custom_tree_list} eq "NA")
	{
		## run msa
		system ("perl $script_path/bulk_msa.pl $temp_pep $config->{program_path}{mafft_path} $temp_msa > $temp_sh/total_msa.sh");
		...
		elsif ($midpoint_rooting eq "Y")
		{
			## convert unrooted to midpoint and rename nwk file
			system ("perl $script_path/convert_midpoint_tree.pl $temp_tree $temp_raw_nwk $script_path > $temp_sh/total_midpoint_tree.sh");

			system ("split -d -l $num_perl --additional-suffix=.sh \"$temp_sh\/total_midpoint_tree.sh\" \"$temp_sh\/temp_midpoint_tree\"");
			system ("perl $script_path/run_bulk.pl $temp_sh temp_midpoint_tree > $temp_sh/Run_midpoint_tree.sh");

			system ("sh $temp_sh/Run_midpoint_tree.sh >$temp_sh/Run_midpoint_tree.sh.log 2>$temp_sh/Run_midpoint_tree.sh.log2");	## bulk
			system ("perl $script_path/validate_run.pl $temp_sh/../hybrid_algorithm.info $temp_raw_nwk nwk >> duplication_validation.log");	
		}
	}
	else
	{
		## copy and rename custom nwk file
		system ("perl $script_path/copy_custom_tree.pl $config->{topology_option}{custom_tree_list} $temp_raw_nwk $temp_sh >> duplication_validation.log");

		system ("sh $temp_sh/total_custom_copy.sh >$temp_sh/total_custom_copy.log 2>$temp_sh/total_custom_copy.log2");
	}
	...
	system ("perl $script_path/bulk_nodeinfo_to_newick.pl $temp_replace nodeinfo_revised $temp_revise_nwk $script_path > $temp_sh/total_multi_revise.sh");
	...
	else
	{
		...
		my $time_string = sprintf("%d hr %d min %d sec", $hr, $min, $sec);
                ...
                open (OUT, ">>duplication_progress.log");
                print OUT "------------------------------------------------------------\n";
                print OUT "[INFO] $timestamp - DupHIST Step1. Applied user custom tree\n";
                print OUT "   - Input:     " . $config->{topology_option}{custom_tree_list} . "\n";
                print OUT "   - Output:    $temp_revise_nwk/*nwk\n";
                print OUT "   - Status:    COMPLETED SUCCESSFULLY ✔\n";
		print OUT "   - Runtime:     $time_string\n";
                print OUT "------------------------------------------------------------\n";
...
if ($step == 2)
{
	...
	## validate tree node
	system ("perl $script_path/validate_tree.pl $config->{required_option}{group_info} $temp_revise_nwk >> duplication_treecheck.log");
	check_tree_log_and_die();
...
if ($step == 3)
{
	...
	system ("perl $script_path/bulk_smooth_process.pl $temp_smooth geneks_1st $temp_smooth $script_path $config->{statistical_option}{stat_lamda} $config->{statistical_option}{stat_alpha} $config->{statistical_option}{start_wraw} > $temp_sh/total_smooth.sh");
...
if ($step == 4)
{
	...
	## build rawks table
	system ("perl $script_path/bulk_build_table_raw.pl $temp_smooth geneks_4th $temp_smooth $script_path > $temp_sh/total_final_raw.sh");

	system ("split -d -l $num_perl --additional-suffix=.sh \"$temp_sh\/total_final_raw.sh\" \"$temp_sh\/temp_final_raw\"");
	system ("perl $script_path/run_bulk.pl $temp_sh temp_final_raw > $temp_sh/Run_final_raw.sh");

	system ("sh $temp_sh/Run_final_raw.sh >$temp_sh/Run_final_raw.log 2>$temp_sh/Run_final_raw.log2");        ## bulk
	
	system ("perl $script_path/validate_run_out.pl $temp_smooth geneks_4th table_raw >> duplication_validation.log");

	# validate log file
	my $has_validation_error = 0;
	if (-e "duplication_validation.log")
	{
		open (my $v_log, "<", "duplication_validation.log");
		while (my $line = <$v_log>) 
		{
			chomp $line;
			next if ($line =~ /^\s*$/);
			if ($line !~ /^-/ && $line !~ /^entire/ && $line !~ /^\s/)
			{
    	...
    	my $status_text = "ALL PROCESSES COMPLETED SUCCESSFULLY ✔";
    	my $finish_text = "Analysis is successfully finished.";
    
    	if ($has_validation_error) 
	{
		$status_text = "COMPLETED WITH WARNINGS (Check duplication_validation.log) ⚠";
		$finish_text = "Analysis finished, but there are some issues in validation.";
    	}
	...
	## copy raw files
	my $header = "Species\tGroup\tNode\tPair1\tPair2\tKs";
	system("printf '$header\n' > $temp_sh/../raw_ks_values.txt");
	system ("cat $temp_smooth/*table_raw >> $temp_sh/../../raw_ks_values.txt");
	...
	if ($has_validation_error)
	{
		print "[DupHIST] WARNING: Please check 'duplication_validation.log' for potential errors.\n";
	}
...
sub check_tree_log_and_die {
    my $log_file = "duplication_treecheck.log";
    my $divider  = "-" x 30;

    return unless -e $log_file;

    open(my $fh, "<", $log_file) or die "[CRITICAL] Cannot open $log_file: $!\n";

    my $has_error = 0;
    my @error_details;

    while (my $line = <$fh>) {
        if ($line =~ /\[ERROR\] Invalid gene IDs or unpruned internodes/) {
            $has_error = 1;
        }

        if ($has_error) {
            push @error_details, $line;
        }
    }
    close($fh);
    ...
    if ($has_error) {
        print "\n============================================================\n";
        print "CRITICAL VALIDATION FAILURE DETECTED IN CUSTOM TREE\n";
        print "============================================================\n";

	print foreach @error_details;

        print "============================================================\n\n";

        die "[FATAL] Pipeline stopped due to custom tree validation error.\n";
    }
}
