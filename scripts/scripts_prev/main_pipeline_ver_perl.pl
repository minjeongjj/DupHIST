...
if ($step == 1)
{
	...
	else
	{
		...
		system ("perl $script_path/copy_custom_tree.pl $config->{topology_option}{custom_tree_list} $temp_raw_nwk $temp_sh >> duplication_customtree.log");

		if (-s "duplication_customtree.log")
		{
			my $divider = "=" x 30;
			die "$divider\n[FATAL] Custom tree check failed: invalid input files.\nPlease correct the problems in duplication_customtree.log before running duphist.\n$divider\n";
		}
		else
		{
...
sub check_log_and_die {
    my $log_file = shift // "duplication_validation.log";
    my $divider  = "=" x 60;

    return unless -e $log_file;

    open(my $log_fh, "<", $log_file) or die "[CRITICAL] Cannot open $log_file: $!\n";
    my $has_error = 0;
    my @error_lines;

    while (my $line = <$log_fh>) {
        if ($line =~ /ERROR|MISMATCH/i) {
            $has_error = 1;
    ...
    if ($has_error) {
        print "\n$divider\n";
        print "[PIPELINE TERMINATED] Critical error detected in validation log.\n";
        print "Log File: $log_file\n";
        print "$divider\n";
        
        my $count = 0;
        foreach my $err (@error_lines) {
            chomp $err;
            print "  -> $err\n";
            last if ++$count >= 5;
        }
        ...
        die "[FATAL] Pipeline stopped due to errors in $log_file.\n";
    }
}
